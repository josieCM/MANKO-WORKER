...
// POC: single-session manager, designed around a session map so more
// sessions can be added later without restructuring.

const CRASH_RECOVERY_BACKOFF_MS = [5000, 15000];

class SessionManager {
  constructor(cfg, logger, base44, events) {
    this.cfg = cfg;
    this.logger = logger;
    this.base44 = base44;
    this.events = events;
    this.sessions = new Map(); // sessionId -> record
    // Dedup guard: a command_id must never execute twice.
  ...
  getSessionConfig(sessionId) {
    return this.cfg.sessions.find((s) => s.sessionId === sessionId) || null;
  }
  ...
  // URL allowlist: navigation is only allowed to configured prefixes.
  isUrlAllowed(sessionId, url) {
    if (!url || typeof url !== "string" || !/^https?:\/\//i.test(url)) return false;
    const s = this.getSessionConfig(sessionId);
    if (!s) return false;
    return s.allowedUrlPrefixes.some((prefix) => url.toLowerCase().startsWith(prefix.toLowerCase().replace(/\/+$/, "") + "/".repeat(0)) || url.toLowerCase().startsWith(prefix.toLowerCase()));
  }

  _resolveUrl(sessionId, suppliedUrl) {
    const s = this.getSessionConfig(sessionId);
    const url = suppliedUrl || s.targetUrl;
  ...
  async startSession(sessionId, opts) {
    ...
    // Idempotent: START on a running session is a no-op, never a second browser.
    ...
    if (activeCount >= this.cfg.maxSessions) {
      const err = new Error(
        `POC limit reached: this worker supports ${this.cfg.maxSessions} session(s); ${activeCount} already active`
      );
    ...
    await rec.browser.launch(sessionId);
    ...
    // Observable authentication check (no automation, no bypass).
  ...
  async stopSession(sessionId, opts) {
    ...
    if (rec.heartbeat) {
      if (o.finalHeartbeatStatus !== false) {
        // One final heartbeat marks the session stopped in the control plane.
  ...
  async restartSession(sessionId, opts) {
    const rec = this._record(sessionId);
    // Clean stop (no 'stopped' final heartbeat - restart ends 'starting'),
    // profile preserved, then start again.
  ...
  async reconnectSession(sessionId) {
    ...
    // Try to recover the page within the surviving context.
    ...
    if (!snap.pageAlive) {
      ...
      const s = this.getSessionConfig(sessionId);
      await this.events.sessionEvent(sessionId, "PAGE_OPENED", {
        message: "Reconnecting: navigating to " + s.targetUrl,
      });
      await rec.browser.navigate(s.targetUrl);
  ...
  getSessionHealth(sessionId) {
    ...
    if (!rec) return null;
    // Synchronous, cheap: browser/page liveness from Playwright handles;
    // URL/title are refreshed on navigation and by the async probe below.
  ...
  // Async refresh used by heartbeat ticks: page probe + auth detection.
  // Returns the browser probe snapshot (incl. latencyMs) or null.
  async refreshSessionState(sessionId) {
    const rec = this.sessions.get(sessionId);
    if (!rec || !rec.browser.isRunning()) return null;
    const snap = await rec.browser.snapshot();
    if (snap.pageAlive) {
      // Detect manual login/logout transitions the operator performs.
      const auth = await rec.browser.detectAuthState();
      ...
      // A ready, healthy, loaded session is active.
  ...
  async _onUnexpectedClose(sessionId) {
    ...
    // Bounded recovery - never an uncontrolled restart loop.
    while (rec.crashRecoveryAttempts < this.cfg.crashMaxRecoveryAttempts) {
      ...
      try {
        await rec.browser.close(); // clear any half-dead handles first
